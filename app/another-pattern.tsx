...
export default function AnotherPatternScreen() {
  ...
  const handleProcessPatterns = async () => {
    setIsLoading(true)
    try {
      const result = await SMSService.getInstance().getDistinctSMSMessagesLastNDays(30)
  ...
  const renderStats = () => {
    ...
    return (
      <View style={styles.statsContainer}>
        <Text style={styles.statsTitle}>📊 Alternative Service Statistics</Text>
  ...
  return (
    <ScrollView style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.title}>🔬 Alternative SMS Patterns</Text>
      ...
      >
        {isLoading ? (
          ...
          <Text style={styles.processButtonText}>🔍 Analyze Last 30 Days</Text>
        )}
      </TouchableOpacity>
      ...
      {patterns.length === 0 && !isLoading && (
        <View style={styles.emptyState}>
          <Text style={styles.emptyStateText}>
            No patterns found. Tap Analyze Last 30 Days to start processing SMS messages.
          </Text>
        </View>
      )}
    </ScrollView>
